...
pdf_file = input("Enter PDF file name( eg abc.pdf):  ")
...
if text:
    ...
    flashcards = generate_all_flashcards(cleaned_sentences)
    save_flashcards(flashcards, "flashcards.json")
    difficulty = input("Choose difficulty (easy/hard): ").lower()
    selected_cards = filter_flashcards(flashcards, difficulty)
    if selected_cards:
        shuffle_flashcards(selected_cards)
        score = run_quiz(selected_cards)
        percentage = (score / len(selected_cards)) * 100
        print(f"\nFinal Score: {score}/{len(selected_cards)}")
        print(f"Percentage: {percentage:.1f}%")
    else:
        print("No flashcards available for that difficulty.")
